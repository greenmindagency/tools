...
$sql = 'SELECT post_date, title, content, image_size, video_size, images, videos, comments FROM client_calendar WHERE client_id = ? AND post_date BETWEEN ? AND ?';
if ($onlyContent) {
    $sql .= " AND content IS NOT NULL AND TRIM(content) <> '' AND title IS NOT NULL AND TRIM(title) <> ''";
} elseif ($onlyTitle) {
    $sql .= " AND title IS NOT NULL AND TRIM(title) <> ''";
}
