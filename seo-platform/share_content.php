...
$stmt = $pdo->prepare('SELECT short_url FROM content_links WHERE client_id=? AND year=? AND month=?');
$stmt->execute([$client_id,$year,$month]);
$url = $stmt->fetchColumn();
if(!$url){
    $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
    $baseUrl = $scheme.'://'.$_SERVER['HTTP_HOST'].dirname($_SERVER['REQUEST_URI']);
    $full = $baseUrl.'/content.php?client_id='.$client_id.'&year='.$year.'&month='.$month;
    $short = @file_get_contents('https://tinyurl.com/api-create.php?url='.urlencode($full));
    if($short){
        $ins = $pdo->prepare('INSERT INTO content_links (client_id,year,month,short_url) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE short_url=VALUES(short_url)');
